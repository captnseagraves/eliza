...
async function getLatestStatus(roomId: string, baseUrl: string, retries = 0): Promise<string> {
    try {
        ...
        if (data.status === "PENDING" && retries < MAX_RETRIES) {
            console.log(`🔄 [rsvpProvider] Status PENDING, retry ${retries + 1} of ${MAX_RETRIES}`);
    ...
    get: async (
        ...
        try {
            ...
            if (existingData && existingData.length > 0) {
                ...
                console.log(" [rsvpProvider] Found existing data:", cachedData);
            }
